...
- [10. 本地化模板（REF）与 FText](#10-本地化模板ref与-ftext)
- [11. 常见问题](#11-常见问题)
...
## 10. 本地化模板（REF）与 FText

本节是 **`Rich Text Composer Document`** 面向使用者的本地化说明。在开启 **`Use Localized Markup`** 时，可对 **`LocalizedMarkupSource`** 使用虚幻的 **Gather Text** / PO 流程。

### 10.1 两个字段做什么

| 字段 | 作用 |
|------|------|
| **Use Localized Markup** | 为 **true** 时，**Rich Text Composer Display** 与 **打字机** 会取当前文化下的 **`LocalizedMarkupSource`**，与 **`Paragraphs`** 的拷贝做 **合并**：**参与模板的正文**来自该 `FText`；**版式、样式、图片、超链、函数芯片等**仍完全来自 **`Paragraphs`**。为 **false** 时只用 **`Paragraphs`**，不合并。 |
| **Localized Markup Source** | 单个 **`FText`**，在模态编辑器点 **OK** 时由插件 **生成**。采用 **稀疏**规则：每个参与本地化的 **正文 Text** run 对应一段 **`{n:i}…{/n:i}`**（见下），**包括** 用于样式/芯片边界的 **空 Text** 占位，便于各文化单独补空格。便于作为 **Gather** 的一条源文。 |

**Gather（Localization Dashboard）：** 虚幻只会收集带 **本地化身份**（namespace + key）的 `FText`。开启 **Use Localized Markup** 后，插件会为该字段写入可收集的身份（每条文档字段用内部 **GUID** 保持稳定）。同时，目标的 **Gather from Packages → Include Path Wildcards** 必须 **覆盖** 存放蓝图 / 控件的目录（例如 `/Game/...`），否则该资产里的内容 **不会** 被收集。升级插件或首次开启后，请在模态里 **OK**、**保存** 资产，再执行一次 **Gather**。

### 10.2 哪些内容会进这条字符串（稀疏）

插件按顺序遍历文档（所有段落与 run），**凡满足**下面条件的 run 都会生成一段模板：

- run 类型为 **`Text`**；  
- **不是** **结构性空** anchor（编辑器为光标在芯片等处插入的零宽占位，**不参与**模板、也不承载可见译文）。

**包含**：**`TextContent` 为空**、但 **非结构性** 的 Text run（常见于不同颜色/芯片之间的「间距槽」）；各文化可在对应 **`{n:i}…{/n:i}`** 里留空，或写入 **空格** 以符合该语言的排版习惯。

**不会**写进本字段、只留在 **`Paragraphs`** 里的包括：**换行**、**图片**、**超链接**、**函数芯片**、结构性空 **text**。芯片在运行时返回的 **动态文案** 应在 **别的数据源 / 别的 `FText` 键** 里本地化，**不**在本字段处理。

### 10.3 给翻译看的格式

多段直接拼接，每一段形如：

**`{n:0}`** *正文* **`{/n:0}`** **`{n:1}`** *正文* **`{/n:1}`** …

- **`i`** 为槽位序号 **0、1、2…**，只给「上面规则里会进串」的 **Text** run 编号，**不是**文档里每一个 run 的全局编号。若原文里某段是 **空占位**，**`{n:i}`** 与 **`{/n:i}`** 之间可以没有字符；需要时也可在该段写入 **空格**。  
- **正文**里若要出现字面量的 **`{`** 或 **`}`**，请写成 **`{{`** 或 **`}}`**。  
- 串内 **不** 用额外换行表示段落；段落分界仍在结构化数据里。

**翻译注意：** 只修改 **`{n:i}`** 与 **`{/n:i}`** 之间的文字；**不要改**标签与数字 **`i`**，也 **不要调换** 各段顺序，否则合并会失败，界面会 **回退** 为只用 **`Paragraphs`** 里的原文，直到改对或在模态里 **OK** 重新生成。

### 10.4 编辑器：详情与预览

在 **Details** 中可开关 **Use Localized Markup**，并展开 **Localized template (REF v1, read-only)** 查看当前生成的串。开启时，**内联预览**与运行时走同一套合并逻辑；在编辑器中切换 **Preview Language**（若可用）时，行为应与游戏内切换文化一致。

### 10.5 出错时

若当前字符串 **无法解析**（序号错位、多余字符、缺闭合标签等），插件会 **忽略** 该字符串用于排版，**回退** 为 **`Paragraphs`**。请在 **模态** 中确认内容后点 **OK** 以 **重新生成** 合法模板。
